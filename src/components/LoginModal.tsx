...
export function LoginModal({ data, isOpen, onClose, onAdminLogin, onDependentLogin, onManagerLogin, deviceId = '' }: LoginModalProps) {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if ((isAdminUsernameMatch || isAdminPhoneMatch) && isAdminPasswordMatch) {
      ...
      const customAdmin = (admin as any).deviceId || (admin as any).deviceIds;
      const isCustomAdminMatch = typeof customAdmin === 'string' && customAdmin.trim().toUpperCase() === currentDeviceIdClean;
      const isAdminDeviceValid = ADMIN_DEVICE_IDS.includes(currentDeviceIdClean) || isCustomAdminMatch;

      if (!isAdminDeviceValid) {
        setError(`Acceso denegado: La cuenta de Administrador solo puede abrirse en los dispositivos autorizados (${ADMIN_DEVICE_IDS.join(', ')}). Tu ID de dispositivo actual es: ${deviceId || 'Desconocido'}.`);
        return;
      }
